...
void CalypsoF17MissionDetectedUi::configure(MissionDetectedState& s, bool allow) {
    ...
    { bool wide = s._hdWideLayout; const auto* g = CalypsoF17MissionDetectedGen::layoutForDesign(wide?1280:740, wide?720:360); if (g) { s._window->setX(g->window.x); s._window->setY(g->window.y); s._window->setWidth(g->window.w); s._window->setHeight(g->window.h); } }
