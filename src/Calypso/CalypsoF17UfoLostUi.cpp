...
void CalypsoF17UfoLostUi::collect(CalypsoHdFrameBuilder& builder) const {
    ...
    m.motionDurationMs = CalypsoF17UfoLostGen::kMotionDurationMs;
    m.motionScaleFrom = CalypsoF17UfoLostGen::kMotionScaleFrom;
    { CalypsoSmallConfirmationButton b{}; b.widget = _state->_btnOk; b.text = b.widget ? b.widget->getText() : std::string(); b.rect = b.widget ? CalypsoLogicalRect{b.widget->getX(), b.widget->getY(), b.widget->getWidth(), b.widget->getHeight()} : proj(g->window); b.tone = CalypsoActionTone::Safe; for(int i=0;i<CalypsoF17UfoLostGen::kButtonCount;++i) if(std::string(CalypsoF17UfoLostGen::kButtons[i].id)=="ok") { b.restFill=CalypsoF17UfoLostGen::kButtons[i].fill; b.restBorder=CalypsoF17UfoLostGen::kButtons[i].border; b.textColor=CalypsoF17UfoLostGen::kButtons[i].text; break; } m.buttons.push_back(b); }
...
void CalypsoF17UfoLostUi::configure(UfoLostState& s, bool allow) {
    ...
    { bool wide = s._hdWideLayout; const auto* g = CalypsoF17UfoLostGen::layoutForDesign(wide?1280:740, wide?720:360); if (g) { s._window->setX(g->window.x); s._window->setY(g->window.y); s._window->setWidth(g->window.w); s._window->setHeight(g->window.h); } }
