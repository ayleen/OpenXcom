	...
	void CalypsoGeoscapeHdGlobeDirect::drawMarkerPass(Globe* globe)
	{
		...
		for (const auto& command : globe->_gpuState->_gpuMarkerCommittedDraws)
		{
			...
			if (command.contactColor)
			{
				...
				const float centerX = static_cast<float>(projection.originX
					+ (command.x + command.frame->getWidth() / 2) * projection.scaleX);
				const float centerY = static_cast<float>(projection.originY
					+ (command.y + command.frame->getHeight() / 2) * projection.scaleY);
				...
				drawQuad(centerX - extent * projection.surfaceScaleX * 0.5f,
					centerY - extent * projection.surfaceScaleY * 0.5f,
					extent * projection.surfaceScaleX, extent * projection.surfaceScaleY);
			}
			globe->_gpuState->_markerShader->use();
			const float x = static_cast<float>(projection.originX + command.x * projection.scaleX);
			const float y = static_cast<float>(projection.originY + command.y * projection.scaleY);
