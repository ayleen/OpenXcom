...
struct CalypsoF21NameLayout{ int designWidth=0,designHeight=0; CalypsoF21Rect window,status,warning,title,message,footer; CalypsoF21Rect inputFrame,inputHint; CalypsoF21Rect ok; CalypsoF21Rect cancel; };
inline CalypsoF21NameLayout calypsoF21NameLayout(CalypsoLayoutClass cls){
 ...
 for(int k=0;k<CalypsoF21NameGen::kButtonCount;++k){ auto br=CalypsoF21NameGen::kButtonRects[idx][k]; if(std::string(br.id)=="ok") l.ok={br.rect.x,br.rect.y,br.rect.w,br.rect.h}; else if(std::string(br.id)=="cancel") l.cancel={br.rect.x,br.rect.y,br.rect.w,br.rect.h}; }
 return l;
}
inline void calypsoF21NameApplyHarnessShift(CalypsoF21NameLayout& l,bool sideBySide){ if(!sideBySide||l.designWidth!=1280) return; int dx=40-l.window.x; l.window.x+=dx; l.status.x+=dx; l.warning.x+=dx; l.title.x+=dx; l.message.x+=dx; l.footer.x+=dx; l.inputFrame.x+=dx; l.inputHint.x+=dx; l.ok.x+=dx; l.cancel.x+=dx; }
}}
