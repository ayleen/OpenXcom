...
struct CalypsoGeoscapeColoredLineSnapshotKey
{
	...
	/* §16.5 review fix: hoverEnabled is part of the static key because
	 * Globe::drawRadars() uses _hover && Options::globeAllRadarsOnBaseBuild
	 * to decide whether static circles around existing bases are recorded.
	 * Hoverlongitude/latitude are NOT in the key so coordinate movement is
	 * a cache hit.  Enter/exit causes one static rebuild. */
	bool hoverEnabled;
