...
void CalypsoF10ManufactureCheckUi::configure(ManufactureInfoState& s, bool allow) {
    ...
    { bool wide = s._hdWideLayout; const auto* g = CalypsoF10ManufactureCheckGen::layoutForDesign(wide?1280:740, wide?720:360); if (g) { s._window->setX(g->window.x); s._window->setY(g->window.y); s._window->setWidth(g->window.w); s._window->setHeight(g->window.h); } }
