...
struct CalypsoF21DefenseLayout{ int designWidth=0,designHeight=0; CalypsoF21Rect window,status,warning,title,message,footer; CalypsoF21Rect cellR1C1,cellR1C2,cellR2C1,cellR2C2,columnDivider1,rowDivider1; CalypsoF21Rect skip,start,ok; };
...
inline void calypsoF21DefenseApplyHarnessShift(CalypsoF21DefenseLayout& l,bool sideBySide){ if(!sideBySide||l.designWidth!=1280) return; int dx=40-l.window.x; l.window.x+=dx; l.status.x+=dx; l.warning.x+=dx; l.title.x+=dx; l.message.x+=dx; l.footer.x+=dx; l.cellR1C1.x+=dx; l.cellR1C2.x+=dx; l.cellR2C1.x+=dx; l.cellR2C2.x+=dx; l.columnDivider1.x+=dx; l.rowDivider1.x+=dx; l.skip.x+=dx; l.start.x+=dx; l.ok.x+=dx; }
}}
