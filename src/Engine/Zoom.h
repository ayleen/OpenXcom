...
class Zoom
{
	...
	/// Returns true iff the OpenGL path actually swapped the window; the
	/// software-zoom paths only fill `dst` and always return false.
	static bool flipWithZoom(SDL_Surface *src, SDL_Surface *dst, int topBlackBand, int bottomBlackBand, int leftBlackBand, int rightBlackBand, OpenGL *glOut, SDL_Window *window = nullptr);
